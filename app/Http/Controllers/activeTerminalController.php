...
class activeTerminalController extends Controller
{
  public function listInactiveTerminalFiltered(Request $request)
  {
    try
    {
      ...
      switch ($range)
      {
          case 'd':
           ...
           case 'm':
              ...
              {
                $twoweek_date = date('Ymd', strtotime($i. ' + 14 days'));
                $filename = 'InactiveEDCReport_'.$i.'-'.$twoweek_date;
      ...
      foreach($arrselected as $key => $value)
      {
        ...
        if($reportType == 'InactiveEDCReport')
        {
          $arrgoodi[$num] = $goodi;
        }
        $num++;
