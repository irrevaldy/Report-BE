...
class monthlyMerchantController extends Controller
{
  public function listMonthlyByAcquirerFiltered(Request $request)
  {
    try
    {
           ...
           case 'm':
              ...
              for($i=$first_date; $i<=$last_date; $i++)
              {
                foreach($merchant as $key => $value)
                  ...
                  $filename = 'MerchantMonthlyRevenueByAcquirer_'.$i.'_'.$merchanta;
                  //$filename = 'ReconsiliationReport_'.$dateFile."_".$username;
      ...
      foreach($arrselected as $key => $value)
      {
        ...
        if($reportType == 'MerchantMonthlyRevenueByAcquirer')
        {
          $arrgoodi[$num] = $goodi;
        }
        $num++;
